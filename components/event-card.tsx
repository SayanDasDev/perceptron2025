  ...
  return (
    <div className="flex flex-col w-[90dvw] h-[90dvh] border-8 border-primary rounded-[2rem] p-8 overflow-clip event-card-shadow">
      <div className="md:flex md:gap-4">
        <div className={`${sub.className} font-size-sm text-primary`}>
          PERCEPTRON 2025
        </div>
        <div
          className={`${handwriting.className} text-2xl leading-3 mt-2 font-light text-slate-400/50`}
        >
          presents
        </div>
      </div>
      <div
        className={`${mono.className} uppercase font-size-lg mt-4 text-slate-300`}
      >
        {title}
      </div>
      <div className="flex-1 mt-4 grid grid-cols-[auto_2rem]">
        <div className="w-full h-full flex flex-col gap-6 overflow-hidden font-size-xs font-extralight">
          <div className="leading-5 text-lg font-normal text-slate-400">
            {description}
          </div>
        ...
        <div className="flex flex-col gap-1 items-end w-full h-full text-xs">
          ...
          <div className={`${sub.className} py-5 text-right text-slate-400/80`}>
            <span className="text-vertical text-slate-500/50">EVENT</span> #
            {eventNumber}
          </div>
